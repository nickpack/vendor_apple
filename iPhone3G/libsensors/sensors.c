...
static inline int sensors_open(const struct sensors_data_device_t *dev, struct native_handle_t* hd)
{
...
static inline int sensors_close(const struct sensors_data_device_t *dev)
{
    if (event_fd > 0) {
        close(event_fd);
        event_fd = -1;
